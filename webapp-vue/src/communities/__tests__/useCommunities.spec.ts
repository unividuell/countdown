...
describe('resolveLanding', () => {
  ...
  it('none when no active communities', () =>
    expect(resolveLanding([], null)).toEqual({ kind: 'none' }))
  it('one when exactly one', () =>
    expect(resolveLanding([a], null)).toEqual({ kind: 'one', slug: 'a' }))
