...
describe('scoreRows', () => {
  ...
  it('labels the reached stage bare and with one decimal, so every comma lines up', () => {
    const rows = rowsOf([
    ...
    expect(rows.find((r) => r.userId === 'a')!.timeLabel).toBe('0,1')
    expect(rows.find((r) => r.userId === 'b')!.timeLabel).toBe('8,0')
    expect(rows.find((r) => r.userId === 'c')!.timeLabel).toBe('15,0')
  })
