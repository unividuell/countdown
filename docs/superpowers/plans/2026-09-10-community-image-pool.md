        ...
        while true; do
          ...
          if [ -n "$$(psql -h postgres -U admin -d app -tAc "SELECT to_regclass('imagepool.images')")" ]; then
            ...
            if [ "$$fingerprint" != "$$(cat /backups/images.fingerprint 2>/dev/null)" ]; then
              # Write beside the name, then rename. A rename within one filesystem is atomic, so a
              # crash mid-dump leaves a .part file that no restore looks at, instead of a truncated
              # images-*.sql.gz that "newest wins" would happily read. The daily dump above can
              # afford not to do this -- its window is time-based, so a bad file ages out within a
              # week either way. This one's window counts CHANGES, so a bad file would hold its
              # slot until the pool moves that many more times, which may be months.
              stamp=$$(date +%Y%m%d-%H%M%S)
              pg_dump -h postgres -U admin -d app --data-only --table=imagepool.images \
                | gzip > "/backups/images-$$stamp.sql.gz.part"
              mv "/backups/images-$$stamp.sql.gz.part" "/backups/images-$$stamp.sql.gz"
              ...
              # The guard is against 0, which an operator may reasonably read as "keep everything"
              # while `tail -n +1` would in fact feed every dump, this run's included, to rm.
              [ "$$IMAGE_BACKUP_KEEP" -ge 1 ] \
