...
## Stumbling block: stale `application-modules.json`

Spring Modulith persists the detected module structure + dependencies to
**`META-INF/spring-modulith/application-modules.json`** (in `core/target/classes/...`),
generated by the AOT processor and by the `Documenter` (our modularity test). The runtime —
including the **module-Flyway strategy** (module identifiers + ordering) — reads it.

It can go **stale** when you change the module set or dependencies (e.g. adding `community`,
or making one module depend on another): the file still describes the old structure, so a new
module may be unrecognized, its migrations may not run, or the ordering is wrong. Symptoms:
"my new module's migrations didn't apply", a cross-schema FK fails because the dependency order
looks wrong, or a freshly added module is missing at runtime.

**Refresh it:** run `./mvnw clean` (drops the stale `target`) and rebuild, and/or re-run the
modularity test so the `Documenter` regenerates it. After adding/repointing a module, prefer a
clean build before trusting runtime/migration behavior.
