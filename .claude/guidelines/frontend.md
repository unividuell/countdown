...
- **Dev proxy:** Vite `server.proxy` forwards `/api`, `/oauth2`, `/login`, `/logout` to the backend (`VITE_API_PROXY_TARGET`, default `http://localhost:8080`) so same-origin holds locally.
